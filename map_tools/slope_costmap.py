...
# map_server sentinel for "no information". Written to the PGM as pixel 255,
# which `mode: raw` maps directly to occupancy -1.
UNKNOWN_OCC = -1
UNKNOWN_PIXEL = 255
...
def occupancy_to_raw_pixels(occ):
    """ROS occupancy 0..100 (plus -1 unknown) -> map_server `mode: raw` PGM pixels.

    THE PGM IS **NOT** INVERTED under `mode: raw`: map_server passes the pixel
    byte straight through as the occupancy value (0 -> free, 100 -> lethal),
    except that pixel 255 is special-cased to occupancy -1 (unknown). This is
    the opposite convention from trinary/scale mode (and from
    map_tools/occupancy_grid.py, which targets trinary) -- do not reuse this
    encoding with a YAML that declares scale or trinary, and do not invert it.

    Unknown (-1) is written as pixel 255, which `mode: raw` maps directly to
    occupancy -1. This is the fix for a measured bug: writing 205 under
    `mode: scale` made every no-terrain cell (water, off-map) carry a fake
    slope cost of 20 instead of publishing as unknown.
    """
    occ = np.asarray(occ)
    px = np.clip(occ, 0, 100).astype(np.uint8)
...
def write_yaml(path, image_name, grid, meta):
    ...
    with open(path, "w") as fh:
        ...
        fh.write("# Pixel 0 = free (<warn_deg), 100 = lethal (>=lethal_deg),\n")
        fh.write("# 255 = unknown (no mesh coverage). Values between ramp\n")
        fh.write("# linearly across the warn..lethal band. NOT inverted:\n")
        fh.write("# pixel value == occupancy value (mode: raw semantics).\n")
        ...
        fh.write("negate: 0\n")
        # occupied_thresh/free_thresh are parsed by map_server but UNUSED in
        # raw mode -- they only apply to trinary/scale. Left in for a reader
        # who expects them; do not assume they gate anything here.
        fh.write("occupied_thresh: 0.65\n")
        fh.write("free_thresh: 0.196\n")
        # mode: raw is REQUIRED. map_server's DEFAULT mode is `trinary`, which
        # collapses every pixel to one of three values: >occupied_thresh ->
        # 100, <free_thresh -> 0, and EVERYTHING IN BETWEEN -> -1 (unknown).
        # That destroys the 10-18 deg ramp before costmap_2d ever sees it --
        # measured: a PGM holding 102 distinct pixel values published as just
        # {-1, 0, 100}. `scale` was tried next and fixed the ramp but broke
        # the unknown sentinel: under scale's occ = round((255-px)/255*100)
        # formula, pixel 205 (the old sentinel) decodes to occupancy 20, a
        # real cost -- measured live: every water/off-map cell published a
        # fake slope cost of 20, and zero cells published as unknown. `raw`
        # passes the pixel straight through as the occupancy value (with 255
        # special-cased to -1), which is why occupancy_to_raw_pixels writes
        # occupancy directly with NO inversion and unknown as 255, not 205.
        fh.write("mode: raw\n")
...
def build(world, maps_dir="maps", warn_deg=10.0, lethal_deg=18.0):
    ...
    write_pgm(base + ".pgm", occupancy_to_raw_pixels(occ_dst))
