"""Mask the object occupancy map to the terrain (DTM) footprint.

Primary use (--mask-unknown --no-crop --erode): write pixel 205 (UNKNOWN) at
every cell the DTM has no terrain for -- NaN height, or outside the DTM
footprint entirely -- so move_base's global costmap sees NO_INFORMATION there
and, with NavfnROS allow_unknown:false, refuses to plan through it. This is
how open water (the lake) and off-mesh void are made no-go WITHOUT declaring
them lethal obstacles: there is no wall there, there is simply no ground
information, and driving in means sinking rather than colliding.

--erode additionally grows that unknown region inward by a true Euclidean
margin, because costmap_2d's InflationLayer does NOT inflate NO_INFORMATION
cells -- without it the planner will put the robot's centre right on the
terrain edge and let the footprint overhang the void.

Secondary use (default, no --no-crop): crop the object map's PGM to the DTM's
world-coordinate footprint, snapping the crop to the object map's OWN cell
grid so no cell is resampled or shifted -- a pure integer crop, never
interpolation. Do NOT use this for the costmap: the masked map must stay on
the same grid as the plain object map, hence --no-crop.
...
from map_tools.dtm_raster import read_dtm_yaml, read_pgm_dimensions
...
def read_pgm(path):
    ...
    # internal convention (occupancy_grid.py).
...
def build(world, maps_dir="maps", out_suffix="_clipped", dry_run=False,
    ...
    if no_crop:
        ...
        # what the ROS costmap needs: the masked map replaces the plain
        # object map on /map, and anything else reading the pair (RViz
        # overlays, the object list, future layers) assumes one shared
        # grid. A crop here would silently shift every cell.
        clipped = pixels
        result = ClipResult(
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Mask a world's object occupancy map to its terrain "
                    "(DTM) footprint. For the move_base global costmap use "
                    "--mask-unknown --no-crop --erode 1.0 --out-suffix "
                    "_terrain, which marks off-terrain cells UNKNOWN (not "
                    "lethal) on the object map's own grid.")
