...
# describe_region's arrangement-grid resolution. These match describe_region's
# defaults today, but we pin them here and pass them EXPLICITLY so the descriptor
# meaning is fixed by this module (not by a library default that could change),
# and so they can be written verbatim into park_regions.yaml's _meta. The
# runtime side (T19) reads _meta and must describe its live window with the SAME
# radius/sectors/rings, else it silently compares descriptors whose arrangement
# rings span different physical radii -- region_distance's length guard cannot
# catch that (the vectors are the same 168 length either way).
N_SECTORS = 8
N_RINGS = 3
    ...
    while y <= ymax:
        ...
        while x <= xmax:
            ...
            if win.shape[0] >= MIN_WINDOW_PTS:
                desc = describe_region(win, n_sectors=N_SECTORS,
                                       n_rings=N_RINGS, radius=radius)
...
def build_regions(models, step=GRID_STEP, radius=WINDOW_RADIUS):
    """Build the distinctive-locations map.

    Returns (regions_dict, meta_dict, report_str).

    regions_dict: {loc_id: {x, y, descriptor: [...168...], nearest: float}} for
    distinctive locations only. meta_dict: the describe_region parameters the
    descriptors were built with (see _write_regions_yaml's _meta). report_str:
    the measured distribution + chosen threshold, printed by main so the choice
    is auditable.
    """
    t0 = time.time()
    ...
    meta = {
        "window_radius": float(radius),
        "grid_step": float(step),
        "n_sectors": int(N_SECTORS),
        "n_rings": int(N_RINGS),
    }
    return regions, meta, report


def _write_regions_yaml(regions, meta, path, generator="extract_park_map.py"):
    """Write park_regions.yaml deterministically (sorted loc_id for stable diffs).

    The file opens with a reserved `_meta` mapping recording the describe_region
    parameters the descriptors were built with. The runtime detector (T19) MUST
    read _meta.window_radius (and n_sectors/n_rings) and describe its live window
    with the same values -- region_distance's length guard cannot catch a radius
    mismatch, since a window at any radius yields the same 168-length vector.
    `_meta` is reserved: location ids are `loc_NNN` and never start with an
    underscore, so no location can collide with it.
    """
    with open(path, "w") as fh:
        fh.write("# Distinctive locations for per-region localization, map-frame "
                 "metres.\n# Generated by %s --regions.\n"
                 "# _meta records the describe_region build parameters; the "
                 "runtime MUST match them.\n"
                 "# Location ids are loc_NNN and never start with '_', so _meta "
                 "cannot collide with one.\n" % generator)
        fh.write("_meta: {window_radius: %s, grid_step: %s, n_sectors: %d, "
                 "n_rings: %d}\n"
                 % (repr(meta["window_radius"]), repr(meta["grid_step"]),
                    meta["n_sectors"], meta["n_rings"]))
...
def main(argv=None):
    ...
    if args.regions:
        regions, meta, report = build_regions(models)
        print(report)
        _write_regions_yaml(regions, meta,
                            os.path.join(args.out_dir, "park_regions.yaml"))
