import os
import sys
import pytest

# Add the tests/operator directory to sys.path so we can import operator_pkg_shim
sys.path.insert(0, os.path.dirname(__file__))
from operator_pkg_shim import load_objects, resolve

def test_load_and_resolve(tmp_path):
    y = tmp_path / "objects.yaml"
    y.write_text("bench_1: {x: 10.0, y: 5.0}\nlamp_2: {x: -3.0, y: 2.0}\n")
    objects = load_objects(str(y))
    assert objects["bench_1"] == (10.0, 5.0)
    gx, gy = resolve("bench_1", objects)
    # No offset by default: resolve returns the object center. Clearance from
    # the object's footprint is handled by snapping in operate.py, not here.
    assert (gx, gy) == (10.0, 5.0)

def test_resolve_unknown_raises_with_names(tmp_path):
    objects = {"bench_1": (1.0, 2.0)}
    with pytest.raises(KeyError) as exc:
        resolve("nope", objects)
    assert "bench_1" in str(exc.value)
